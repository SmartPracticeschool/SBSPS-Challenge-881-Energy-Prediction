...
def index(request):
    if request.method == "POST":
        ...
        if prediction.is_valid():
            ...
            trace = Scatter(x=np.arange(200), y=pred, fill='tonexty', fillcolor='orange',marker={'color':'green'})
            data = Data([trace])
            layout = Layout(title='The  Predicted Wind Power in 72 Hours Future is', xaxis={'title': 'Hours'}, yaxis={'title': 'Power'})
            fig = Figure(data=data, layout=layout)
            fig.update_layout(legend_title=dict(text='Hours', font=dict(color='blue')))
            div = plot(fig, auto_open=False, output_type='div')
            return render(request, 'weather/index.html',{'speed': div, 'form': prediction})
